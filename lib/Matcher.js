...
var compareTermWithSubject = require('./tools/compareTermWithSubject')
    ...
    match (term, overallCount) {
        ...
        for (var i = result.length - 1; i >= 0; i--) {
            if (result[i]) {
                ...
                var groupedSubResult = result[i].map((subject) =>
                    new ResultEntry(subject, i, splittedDeduped.length, compareTermWithSubject(subject, preparedTerm)))
