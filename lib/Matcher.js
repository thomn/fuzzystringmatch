...
class Matcher {
    ...
    match (term, overallCount) {
        ...
        for (var i = result.length - 1; i >= 0; i--) {
            if (result[i]) {
                var groupedSubResult = result[i].slice(0, overallCount - cummulatedResult.length)
                groupedSubResult = groupedSubResult.map((subject) => new ResultEntry(subject, i))

                cummulatedResult = cummulatedResult.concat(groupedSubResult)
                if (cummulatedResult.length >= overallCount) break
            }
        }
